...
public class FireworkEvent implements IEvent<PlayerEvent.PlayerLoggedInEvent> {
    ...
    public FireworkEvent() {
        ...
        try {
            ...
            contributors.forEach(contributor -> {
                ...
                DiaboloLibCommon.LOGGER.debug(new FormattedMessage("Contributor found: %s/%s/%s", UUID, name, type));
            });
        } catch (IOException e) {
            DiaboloLibCommon.LOGGER.error(new FormattedMessage("Error during ContributorEvent event execution: %s", e.getMessage()));
        }
    }
    ...
    private void generateFirework(@NotNull Player player, @NotNull Level world, CompoundTag star, int flight) {
        ...
        DiaboloLibCommon.LOGGER.debug(new FormattedMessage("Spawning [%s] Firework.", player.getName().getString()));
