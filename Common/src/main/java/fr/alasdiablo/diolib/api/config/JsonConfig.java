    ...
    void initOrLoad() throws IOException, IllegalStateException {
        if (filePath == null) {
            ...
            try {
                ...
                this.read(json);
                fileReader.close();
                DiaboloLibCommon.LOGGER.debug(String.format("Config %s have been loaded", this.getName()));
            } catch (FileNotFoundException e) {
                this.preWrite();
                DiaboloLibCommon.LOGGER.debug(String.format("Config %s have been write and loaded", this.getName()));
            } catch (JsonParseException | IllegalArgumentException | NullPointerException e) {
                DiaboloLibCommon.LOGGER.warn(String.format("Error during config initialization on %s cause by: %s", this.getName(), e.getLocalizedMessage()));
                this.preWrite();
                DiaboloLibCommon.LOGGER.debug(String.format("Config %s have been rewrite and loaded", this.getName()));
            }
        }
    }
